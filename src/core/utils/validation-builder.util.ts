...
export class ValidationBuilder {
    ...
    static Required(options: ValidationOptions = {}) {
        this._validators.push(
            check(this._field)
                .exists()
                .withMessage(options.message ?? getMessage(ValidationMessages.required, { field: options.fieldDisplayName ?? this._field }))
        );
        return this;
    }
    ...
    static IsEmail(options: ValidationOptions = {}) {
        this._validators.push(
            check(this._field)
                .isEmail()
                .withMessage(options.message ?? getMessage(ValidationMessages.email, { field: options.fieldDisplayName ?? this._field }))
        );
        return this;
    }
    ...
    private static checkLength(minOrMax: "min" | "max", length: number, options: ValidationOptions = {}) {
        this._validators.push(
            check(this._field)
